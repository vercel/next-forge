...
Environment validation is enabled by default so missing required variables fail fast. If you intentionally deploy a project without those variables, set:

```bash
SKIP_ENV_VALIDATION="true"
```

Only use this when the missing variables are expected for that deployment.
