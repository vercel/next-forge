...
You need [Node.js](https://nodejs.org) 20+ and a package manager. We recommend [Bun](https://bun.sh), but npm, yarn, and pnpm all work.
...
- Set up [Stripe](/packages/payments) for payments
- Set up [Resend](/packages/email) for transactional emails
- Set up [BaseHub](/packages/cms/overview) for CMS content
- Configure [analytics](/packages/analytics/web), [error tracking](/packages/observability/error-capture), and [logging](/packages/observability/logging)
- Read the [deployment](/docs/deployment) guide when you're ready to go live
